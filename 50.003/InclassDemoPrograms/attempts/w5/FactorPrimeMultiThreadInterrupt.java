...
//		BigInteger n = new BigInteger("1127451830576035879");
    ...
    public static BigInteger factorMultiThread (BigInteger input) {
        FactorWithInterrupt[] factors = new FactorWithInterrupt[numberOfThreads];

        for (int i = 0; i < numberOfThreads; i++) {
            factors[i] = new FactorWithInterrupt(input, i+2, numberOfThreads);
            factors[i].start();
        }
...
class FactorWithInterrupt extends Thread {
    BigInteger n;
    BigInteger init;
    BigInteger stepSize;
    BigInteger result;

    public FactorWithInterrupt (BigInteger n, int init, int stepSize) {
        this.n = n;
